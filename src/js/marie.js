...
(function () {
  ...
  MarieSim.prototype.regSet = function (target, source, mask, alu_type) {
    ...
    if (source == 'm') {
      ...
      const oldCell = this.memory[this.mar].contents;
      ...
      const src = typeof source == 'string' ? this[source] : source;
      const msk = mask !== undefined ? mask : 0xFFFF;
  ...
  MarieSim.prototype.decode = function () {
    const opcode = Utility.intToUint(this.ir) >> 12;

    for (const op in MarieSim.prototype.operators) {
  ...
  MarieSim.prototype.operators = {
    ...
    input: {
      ...
      * fn() {
        ...
        yield this.inputCallback.call(null, (v) => {
          myself.paused = false;
          value = v;
        });
  ...
  MarieAsm.prototype.assemble = function () {
    ...
    for (i = 0; i < lines.length; i++) {
      ...
      const originationDirective = line.match(/^\s*org\s+([0-9a-f]{3})\s*(?:\/.*)?$/i);
      ...
      if (label) {
        ...
        if (label in symbols) {
          const entry = parsed.filter(checkLabel.bind(null, label));
    ...
    for (i = 0; i < parsed.length; i++) {
      ...
      if (directiveBase) {
        ...
        let constant = parseInt(instruction.operand, directiveBase);
        if (isNaN(constant)) {
          throw new MarieAsmError(
                        'Syntax error',
                        instruction.line,
